    ...
    switch (NewStatus)
    {
...
#ifdef IVI_DEBUG
            printk("UpdateTcpStateContext: ignore packet on map %d -> %d, state %d\n", 
...
#ifdef IVI_DEBUG
            printk("UpdateTcpStateContext: invalid packet on map %d -> %d, state %d, drop packet and clear state.\n", 
                      StateContext->oldport, StateContext->newport, OldStatus);
#endif
            return FILTER_DROP_CLEAN;
        ...
        case TCP_STATUS_CLOSE:
                ...
                && before(ntohl(th->seq), receiver->MaxAck))
            {
                // Invalid RST
#ifdef IVI_DEBUG
                printk("UpdateTcpStateContext: invalid RST packet on map %d -> %d, state %d, drop packet.\n", 
                          StateContext->oldport, StateContext->newport, OldStatus);
#endif
                return FILTER_DROP;
            }
...
#ifdef IVI_DEBUG
			printk("refresh_tcp_map_list: map %d -> %d time out on TCP state %d\n", iter->oldport, iter->newport, iter->Status);
#endif
			kfree(iter);
		}
	}
...
void free_tcp_map_list(void)
{
	...
	list_for_each_entry_safe(iter, temp, &tcp_list.chain, node) {
		...
		tcp_list.size--;
#ifdef IVI_DEBUG
		printk("free_tcp_map_list: map %d -> %d deleted on TCP state %d\n", iter->oldport, iter->newport, iter->Status);
#endif
		kfree(iter);
	}
	memset(tcp_list.used, 0, 65536 * sizeof(__u8));
	...
	*newp = 0;
		...
		list_for_each_entry_safe(StateContext, temp, &tcp_list.chain, node) {
			if (StateContext->oldport == oldp) {
				// Update state context.
...
#ifdef IVI_DEBUG
                			printk("get_outflow_tcp_map_port: Found map %d -> %d, TCP state %d\n", 
...
#ifdef IVI_DEBUG
                			printk("get_outflow_tcp_map_port: Invalid packet on map %d -> %d, TCP state %d\n", 
						StateContext->oldport, StateContext->newport, StateContext->Status);
#endif
                		}
                		...
                		{
#ifdef IVI_DEBUG
                			printk("get_outflow_tcp_map_port: Invalid state on map %d -> %d, TCP state %d\n", 
						StateContext->oldport, StateContext->newport, StateContext->Status);
#endif
	...
	if (retport == 0) // No existing map, generate new map
	{
		...
		if (ratio == 1) {
			...
			if (remaining <= 0) {
				spin_unlock_bh(&tcp_list.lock);
				printk("get_outflow_tcp_map_port: failed to assign a new map port for %d\n", oldp);
				return -1;
			}
		}
	...
	StateContext = (PTCP_STATE_CONTEXT)kmalloc(sizeof(TCP_STATE_CONTEXT), GFP_ATOMIC);
        if (StateContext == NULL)
        {
            	...
            	spin_unlock_bh(&tcp_list.lock);
#ifdef IVI_DEBUG
            	printk("get_outflow_tcp_map_port: kmalloc failed for map %d\n", oldp);
#endif
		return -1;
        }
        memset(StateContext, 0, sizeof(TCP_STATE_CONTEXT));
	...
	ftState = CreateTcpStateContext(th, len, StateContext);
        ...
        if (ftState == FILTER_DROP_CLEAN)
        {
            	spin_unlock_bh(&tcp_list.lock);
#ifdef IVI_DEBUG
            	printk("get_outflow_tcp_map_port: Invalid state on %d, TCP state %d, fail to add new map.\n", 
...
#ifdef IVI_DEBUG
        printk("get_outflow_tcp_map_port: New map %d -> %d added, TCP state %d\n", 
                StateContext->oldport, StateContext->newport, StateContext->Status);
#endif
    	}
        	...
        	if (StateContext->newport == newp)  // Found existing mapping info
        	{
            		...
            		if (ftState == FILTER_ACCEPT)
            		{
                		*oldp = StateContext->oldport;
                		ret = 0;
#ifdef IVI_DEBUG
                		printk("get_inflow_tcp_map_port: Found map %d -> %d, TCP state %d\n", 
					StateContext->oldport, StateContext->newport, StateContext->Status);
#endif
            		}
            		else if (ftState == FILTER_DROP)
            		{
                		// Return -1 to drop current segment, keep the state info.
#ifdef IVI_DEBUG
               			 printk("get_inflow_tcp_map_port: Invalid packet on map %d -> %d, TCP state %d\n", 
				 	StateContext->oldport, StateContext->newport, StateContext->Status);
#endif
            		}
            		else  // FILTER_DROP_CLEAN
            		{
#ifdef IVI_DEBUG
                		printk("get_inflow_tcp_map_port: Invalid state on map %d -> %d, TCP state %d\n", 
					StateContext->oldport, StateContext->newport, StateContext->Status);
#endif
                	// Remove state info, return -1
                	list_del(&StateContext->node);
