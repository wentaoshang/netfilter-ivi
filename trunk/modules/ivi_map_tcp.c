    ...
    while (optlen > 0)
    {
        ...
        switch (optcode)
        {
            ...
            default:
                ...
                if (optcode == TCPOPT_SACK 
                    && optsize >= (TCPOLEN_SACK_BASE + TCPOLEN_SACK_PERBLOCK)
...
static bool tcp_in_window(struct tcphdr *th, __u32 len, PACKET_DIR dir, PTCP_STATE_CONTEXT StateContext)
{
    ...
    */
    ...
    else if (((tcp_flag_word(th) & (TCP_FLAG_ACK|TCP_FLAG_RST)) ==
...
#ifdef IVI_DEBUG
    printk("tcp_in_window: seq = %u, ack = %u, sack = %u, win = %u, end = %u\n", seq, ack, sack, win, end);
    printk("tcp_in_window: sender end=%u maxend=%u maxwin=%u scale=%u\n", 
              sender->End, sender->MaxEnd, sender->MaxWindow, sender->Scale);
    printk("tcp_in_window: receiver end=%u maxend=%u maxwin=%u scale=%u\n", 
              receiver->End, receiver->MaxEnd, receiver->MaxWindow, receiver->Scale);
    printk("tcp_in_window: I=%d II=%d III=%d IV=%d\n",
...
#ifdef IVI_DEBUG
        printk("CreateTcpStateContext: invalid new packet causing state change to %d, drop.\n", NewStatus);
#endif
...
FILTER_STATUS UpdateTcpStateContext(struct tcphdr *th, __u32 len, PACKET_DIR dir, PTCP_STATE_CONTEXT StateContext)
{
    ...
    switch (NewStatus)
    {
...
#ifdef IVI_DEBUG
            printk("UpdateTcpStateContext: ignore packet on map %x:%d -> %d, state %d\n", StateContext->oldaddr, 
...
#ifdef IVI_DEBUG
            printk("UpdateTcpStateContext: invalid packet on map %x:%d -> %d, state %d, drop packet and clear state.\n", StateContext->oldaddr,  
                      StateContext->oldport, StateContext->newport, OldStatus);
#endif
            return FILTER_DROP_CLEAN;
        ...
        case TCP_STATUS_CLOSE:
                ...
                && before(ntohl(th->seq), receiver->MaxAck))
            {
                // Invalid RST
#ifdef IVI_DEBUG
                printk("UpdateTcpStateContext: invalid RST packet on map %x:%d -> %d, state %d, drop packet.\n", StateContext->oldaddr,  
                          StateContext->oldport, StateContext->newport, OldStatus);
#endif
                return FILTER_DROP;
            }
    ...
    StateContext->LastDir = dir;
#ifdef IVI_DEBUG    
    printk("UpdateTcpStateContext: syn=%d ack=%d fin=%d rst=%d old_state=%d new_state=%d\n",
...
void init_tcp_map_list(void)
{
	...
	memset(tcp_list.used, 0, 65536 * sizeof(__u8));
}
...
void refresh_tcp_map_list(void)
{
	...
	list_for_each_entry_safe(iter, temp, &tcp_list.chain, node) {
		...
		if (delta >= iter->StateTimeOut) {
			list_del(&iter->node);
			tcp_list.size--;
			tcp_list.used[iter->newport] = 0;
#ifdef IVI_DEBUG
			printk("refresh_tcp_map_list: map %x:%d -> %d time out on TCP state %d\n", iter->oldaddr, iter->oldport, iter->newport, iter->Status);
#endif
			kfree(iter);
		}
	}
...
void free_tcp_map_list(void)
{
	...
	list_for_each_entry_safe(iter, temp, &tcp_list.chain, node) {
		...
		tcp_list.size--;
#ifdef IVI_DEBUG
		printk("free_tcp_map_list: map %x:%d -> %d deleted on TCP state %d\n", iter->oldaddr, iter->oldport, iter->newport, iter->Status);
#endif
		kfree(iter);
	}
	memset(tcp_list.used, 0, 65536 * sizeof(__u8));
...
int get_outflow_tcp_map_port(__be32 oldaddr, __be16 oldp, struct tcphdr *th, __u32 len, __be16 *newp)
{
	...
	*newp = 0;
	...
	if (tcp_list.size >= (int)(64513 / ratio)) {
		spin_unlock_bh(&tcp_list.lock);
		printk("get_outflow_tcp map_port: map list full.\n");
		return -1;
	}
		...
		list_for_each_entry_safe(StateContext, temp, &tcp_list.chain, node) {
			if (StateContext->oldport == oldp && StateContext->oldaddr == oldaddr) {
				// Update state context.
...
#ifdef IVI_DEBUG
                			printk("get_outflow_tcp_map_port: Found map %x:%d -> %d, TCP state %d\n", StateContext->oldaddr, 
...
#ifdef IVI_DEBUG
                			printk("get_outflow_tcp_map_port: Invalid packet on map %x:%d -> %d, TCP state %d\n", StateContext->oldaddr, 
						StateContext->oldport, StateContext->newport, StateContext->Status);
#endif
                		}
                		...
                		{
#ifdef IVI_DEBUG
                			printk("get_outflow_tcp_map_port: Invalid state on map %x:%d -> %d, TCP state %d\n", StateContext->oldaddr, 
						StateContext->oldport, StateContext->newport, StateContext->Status);
#endif
                    			// Remove state info, return -1
                    			list_del(&StateContext->node);
                    			tcp_list.size--;
                    			tcp_list.used[StateContext->newport] = 0;
                    			kfree(StateContext);
                		}
	...
	if (retport == 0) // No existing map, generate new map
	{
		...
		if (ratio == 1) {
			...
			if (tcp_list.last_alloc != 0)
				rover = tcp_list.last_alloc + 1;
			else
				rover = low;
			
			do { 
				retport = rover * ratio + offset;
				if (!tcp_list.used[retport])
					break;
			...
			if (remaining <= 0) {
				spin_unlock_bh(&tcp_list.lock);
				printk("get_outflow_tcp_map_port: failed to assign a new map port for %x:%d\n", oldaddr, oldp);
				return -1;
			}
		}
	...
	StateContext = (PTCP_STATE_CONTEXT)kmalloc(sizeof(TCP_STATE_CONTEXT), GFP_ATOMIC);
        if (StateContext == NULL)
        {
            	...
            	spin_unlock_bh(&tcp_list.lock);
#ifdef IVI_DEBUG
            	printk("get_outflow_tcp_map_port: kmalloc failed for map %x:%d\n", oldaddr, oldp);
#endif
		return -1;
        }
        memset(StateContext, 0, sizeof(TCP_STATE_CONTEXT));
	...
	ftState = CreateTcpStateContext(th, len, StateContext);
        ...
        if (ftState == FILTER_DROP_CLEAN)
        {
            	spin_unlock_bh(&tcp_list.lock);
#ifdef IVI_DEBUG
            	printk("get_outflow_tcp_map_port: Invalid state on %x:%d, TCP state %d, fail to add new map.\n", oldaddr, 
        ...
        tcp_list.used[retport] = 1;
#ifdef IVI_DEBUG
        printk("get_outflow_tcp_map_port: New map %x:%d -> %d added, TCP state %d\n", StateContext->oldaddr, 
                StateContext->oldport, StateContext->newport, StateContext->Status);
#endif
    	}
...
int get_inflow_tcp_map_port(__be16 newp, struct tcphdr *th, __u32 len, __be32 *oldaddr, __be16 *oldp)
{
	...
	*oldaddr = 0;
	...
	spin_lock_bh(&tcp_list.lock);
    	...
    	if (list_empty(&tcp_list.chain)) {
		spin_unlock_bh(&tcp_list.lock);
		printk("get_inflow_tcp_map_port: map list empty.\n");
		return -1;
	}
        	...
        	if (StateContext->newport == newp)  // Found existing mapping info
        	{
            		...
            		if (ftState == FILTER_ACCEPT)
            		{
                		...
                		*oldp = StateContext->oldport;
                		ret = 0;
#ifdef IVI_DEBUG
                		printk("get_inflow_tcp_map_port: Found map %x:%d -> %d, TCP state %d\n", StateContext->oldaddr, 
					StateContext->oldport, StateContext->newport, StateContext->Status);
#endif
            		}
            		else if (ftState == FILTER_DROP)
            		{
                		// Return -1 to drop current segment, keep the state info.
#ifdef IVI_DEBUG
               			 printk("get_inflow_tcp_map_port: Invalid packet on map %x:%d -> %d, TCP state %d\n", StateContext->oldaddr, 
				 	StateContext->oldport, StateContext->newport, StateContext->Status);
#endif
            		}
            		else  // FILTER_DROP_CLEAN
            		{
#ifdef IVI_DEBUG
                		printk("get_inflow_tcp_map_port: Invalid state on map %x:%d -> %d, TCP state %d\n", StateContext->oldaddr, 
					StateContext->oldport, StateContext->newport, StateContext->Status);
#endif
                	// Remove state info, return -1
                	list_del(&StateContext->node);
